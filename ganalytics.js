...
if(Meteor.settings && Meteor.settings.public !== undefined && Meteor.settings.public.ga !== undefined && Meteor.settings.public.ga.account !== undefined && (!dnt() || (dnt() && !Meteor.settings.public.ga.respectDNT)) ) {
  ...
  GAnalytics.pageview = function(pageLocation) {
    if(dnt() && gaSettings.respectDNT) {
      if(!!gaSettings.debug)
        console.log("Logging pageview blocked by DNT. Page was: "+pageLocation)
      return;
    }
    if(!!gaSettings.debug)
  ...
  GAnalytics.event = function(category, action, label, value) {
    if(dnt() && gaSettings.respectDNT) {
      if(!!gaSettings.debug)
        console.log("Logging event blocked by DNT. Event was: "+category+" | "+ action + " | " + label + " | " + value)
      return;
    }
    if(!!gaSettings.debug)
